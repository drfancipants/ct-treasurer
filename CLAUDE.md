...
**Changing a Sensitive var's value:** its plaintext can never be shown, so you can only set / **rotate** / delete it — not edit it in place in the dashboard. Use the dashboard's **Rotate** action (or `vercel env update <NAME> <env> --sensitive`), *not* delete-and-recreate. **Any env var change (rotate included) requires a redeploy** — it never applies to the running deployment. Safe rotation order when the old secret is still live: rotate in Vercel → redeploy → verify → *then* revoke the old key at the provider (revoking first breaks the live deployment).
...
Removing a bank account (`removeBankAccount` in `actions/bank.ts`) revokes the Plaid Item via `itemRemove` once its **last** account row is deleted (one Item can back several `BankAccount` rows). The delete-and-count runs inside a transaction guarded by a per-Item `pg_advisory_xact_lock` — without it, concurrent removals of accounts on the same Item each count while the other's row still exists, both skip the revoke, and the Item is orphaned (still billing at Plaid).

**OAuth** (large banks — Chase, BofA, etc.): gated behind `PLAID_REDIRECT_URI`. When set, `link-token` passes it as `redirect_uri`; **it must exactly match a URI registered in the Plaid dashboard** (Team Settings → Allowed redirect URIs) or `linkTokenCreate` fails for *every* institution — so leaving it unset safely disables OAuth rather than breaking all linking. The flow: `PlaidLinkButton` persists the link token + committee to `localStorage` (key `PLAID_OAUTH_KEY`) before opening, the bank redirects to the registered `/app/plaid-oauth` page, which resumes Link with `receivedRedirectUri` and runs the normal exchange. Testable in sandbox with the Platypus OAuth bank (`ins_127287`) once the redirect URI is registered.
