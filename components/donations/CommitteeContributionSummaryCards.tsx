...
export default function CommitteeContributionSummaryCards({ contributions }: Props) {
  const total = contributions.reduce((sum, c) => sum + c.amount, 0)
  const filed = contributions.filter((c) => c.filedAt)
  const notFiled = contributions.filter((c) => !c.filedAt)
  const largest = contributions.length ? Math.max(...contributions.map((c) => c.amount)) : 0
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard
        ...
        iconBg="bg-blue-50"
        label="Filed"
        value={formatCurrency(filed.reduce((s, c) => s + c.amount, 0))}
      ...
      <StatCard
        ...
        iconBg="bg-slate-100"
        label="Not yet filed"
        value={formatCurrency(notFiled.reduce((s, c) => s + c.amount, 0))}
      ...
      <StatCard
        ...
        sub={
          largest > 0
            ? contributions.find((c) => c.amount === largest)?.fromName ?? ''
